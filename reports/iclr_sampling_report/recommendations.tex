...
\begin{itemize}
\item Promote the \textbf{MoG mode-count scaling}
(Table~\ref{tab:mog_emc_scaling}) and the \textbf{ManyWell dimension-scaling}
(Table~\ref{tab:manywell_countemc_scaling}) results to the main paper: they show
how coverage / EMC degrade with $K$ and $d$ for the local baselines while LSB-MC
stays close to the reference.
\item Add the \textbf{Bayesian GMM label-switching} experiment
(Table~\ref{tab:bayes_gmm}) as the headline ``real model'' result: the jump bank
is derived from a genuine symmetry of the posterior rather than hand-tuned, which
strengthens the methodological story.
...
\item Report the \textbf{compute-fairness} table (Table~\ref{tab:compute};
wall-clock and gradient/potential evaluations) so the speed-up is not attributed
to extra compute.
\end{itemize}
