...
丙氨酸二肽 & 筛后保留 & FES 盆地之间的环面最短 $(\Delta\varphi,\Delta\psi)$ 纯二面角原子 &
在\emph{内坐标}中做纯二面角转动，使弦 Jacobian-free；弦穿过 Ramachandran 禁区
（打分指数越过 $M_{\max}$）的原子被筛除并记录，未连边的盆地对经中间盆地中继。 \\
\bottomrule
\end{tabularx}
\caption{五个体系的跳跃设计。}
\end{table}
...
\section{E5：丙氨酸二肽（真实力场）}

前四个体系的势能都是解析曲面。E5 把同一套 LSC-CP 机制放到\textbf{真实分子力场}上：
真空中的丙氨酸二肽 Ac--Ala--NHMe（\texttt{AlanineDipeptideVacuum}，
\texttt{constraints=None}，即完全柔性、所有键都是简谐键），22 个原子、
$D=3N=66$ 个直角坐标自由度，
包含 21 个键、36 个键角、52 个周期二面角项和 98 个非键例外（1--2/1--3 排除与
按比例缩放的 1--4 对）。温度 $T=300\,$K，$\beta=1/(k_BT)=0.40091$ mol/kJ，
$k_BT=2.4943$ kJ/mol；$\beta$ 在 E5 内部局部传递，\texttt{config.BETA}$=8$
对 E1/E2/E4 保持不变（与 E3 在 $\beta=24$ 的做法一致）。

慢事件是 $\varphi$ 的\textbf{符号翻转}：负 $\varphi$ 簇（C5/$\beta$、C7eq、$\alpha_R$）
占据约 $99\%$ 的质量，正 $\varphi$ 岛（C7ax、$\alpha_L$）只能越过 Ramachandran
势垒到达。初值置于 C7eq。

\subsection{为什么直角坐标上的直线跳跃必然失败}

盆地之间的跃迁在几何上是分子片段绕 N--C$\alpha$ 轴的一次\emph{转动}，转角记为 $\Delta$。
设被转动片段中某原子到转轴的垂直距离为 $\rho$，真实运动是半径 $\rho$、张角 $\Delta$ 的
\textbf{圆弧}；而直角坐标上的直线跳跃 $x\mapsto x+r$ 走的是这段圆弧的\textbf{弦}。
在弦的参数 $\theta\in[0,1]$ 处，该原子到转轴的距离为
\begin{equation}
\rho\,\bigl|(1-\theta)e^{i\cdot 0}+\theta e^{i\Delta}\bigr|
=\rho\sqrt{(1-\theta)^2+\theta^2+2\theta(1-\theta)\cos\Delta},
\end{equation}
在 $\theta=\tfrac12$ 取到最小值 $\rho\cos(\Delta/2)$，即产生径向亏损
\begin{equation}
\rho\bigl(1-\cos(\Delta/2)\bigr)=O(\Delta^2).
\end{equation}
对 $\Delta\approx140^\circ$，$1-\cos70^\circ\approx0.66$，即弦中点处收缩约 $0.66\rho$。
横跨转动边界的刚性键与键角必须吸收这一收缩，于是弦被推高到盆地之上数百个 $k_BT$，
弦上的打分被积函数 $e^{-\beta[U(x-\theta r)-U(x)]}$ 直接饱和 $M_{\max}$ 帽。
这与 E3 里必须使用中继原子、E4 里必须删除对角原子是同一个障碍，只是在这里它是
\textbf{结构性}的。$O(\Delta^2)$ 的标度同时也给出了出口：把采样搬进内坐标。

\subsection{弦--微积分基本定理（chord--FTC）恒等式}

LSC-CP 的漂移修正为
\begin{equation}
S_{\nu,\beta}(x)=-\lambda\int\rho(\mathrm dr)\,r\int_0^1
\exp\bigl[-\beta\bigl(U(x-\theta r)-U(x)\bigr)\bigr]\mathrm d\theta .
\end{equation}
\textbf{断言（逐原子 $\mu$ 不变性）。} 对固定的单个原子 $r$、跳跃率 $\lambda$，记
$s_r(x)=-\lambda r\int_0^1 e^{-\beta[U(x-\theta r)-U(x)]}\mathrm d\theta$，
单原子修正生成元 $A_rf=\lambda[f(x+r)-f(x)]+s_r\!\cdot\!\nabla f$ 对一切
$f\in C_c^\infty$ 满足 $\int A_rf\,\mathrm d\mu=0$。

\textbf{证明。} 略去常数 $Z$，把 $\int A_rf\,\mathrm d\mu$ 拆成 $I_1+I_2$。

\emph{跳跃项。} 在平移项中作代换 $x\to x-r$，得
$\int f(x+r)e^{-\beta U(x)}\mathrm dx=\int f(x)e^{-\beta U(x-r)}\mathrm dx$，故
\begin{equation}
I_1=\lambda\int f(x)\bigl[e^{-\beta U(x-r)}-e^{-\beta U(x)}\bigr]\mathrm dx .
\end{equation}
\emph{漂移项。} 令 $F(x):=s_r(x)e^{-\beta U(x)}=-\lambda r\,\phi(x)$，其中
$\phi(x):=\int_0^1 e^{-\beta U(x-\theta r)}\mathrm d\theta$（沿 $r$ 方向的向量）。
由紧支撑分部积分 $I_2=\int F\!\cdot\!\nabla f\,\mathrm dx=-\int f(\nabla\!\cdot\!F)\mathrm dx$。
因 $r$ 为常向量，$\nabla\!\cdot\!F=-\lambda(r\!\cdot\!\nabla\phi)$。利用
$\partial_\theta U(x-\theta r)=-r\!\cdot\!\nabla U(x-\theta r)$ 得
\begin{equation}
r\!\cdot\!\nabla_x e^{-\beta U(x-\theta r)}
=-\beta (r\!\cdot\!\nabla U)e^{-\beta U(x-\theta r)}
=\beta(\partial_\theta U)e^{-\beta U(x-\theta r)}
=-\partial_\theta e^{-\beta U(x-\theta r)},
\end{equation}
于是 $r\!\cdot\!\nabla\phi=-\int_0^1\partial_\theta e^{-\beta U(x-\theta r)}\mathrm d\theta
=e^{-\beta U(x)}-e^{-\beta U(x-r)}$，从而
$\nabla\!\cdot\!F=\lambda(e^{-\beta U(x-r)}-e^{-\beta U(x)})$，
$I_2=-I_1$，故 $I_1+I_2=0$。$\square$

这一恒等式对\textbf{任意}位移 $r$ 成立，不需要对 $\rho$ 作任何对称性假设；
对 $\rho$ 求和/积分即得完整弱不变性。它同时说明：对每一个\emph{冻结}的 $R$，
RA 估计量所用的成对跳跃+漂移核都是逐原子 $\mu$ 不变的，
这正是 E5 只需要“沿弦的能量差”而不需要力场的任何解析结构的原因。

\subsection{BAT 内坐标与 Jacobian 的二面角无关性}

把采样搬到 BAT（键长--键角--二面角）内坐标，$\varphi,\psi$ 就\emph{字面上}是两个坐标。
以三个前驱原子为参考放置第 $i$ 个原子，用的 $(b_i,a_i,\tau_i)$ 恰好是以参考原子为心的
\textbf{球坐标}，其直角体积元为 $b_i^2\sin(a_i)\,\mathrm db_i\,\mathrm da_i\,\mathrm d\tau_i$。
对所有被放置的原子作复合（外部平移与转动被固定为边界项），得
\begin{equation}
\mathrm dx=\prod_i b_i^2\sin(a_i)\,\mathrm dq
\quad\Longrightarrow\quad
\Bigl|\det\frac{\partial x}{\partial q}\Bigr|=\prod_i b_i^2\sin(a_i).
\end{equation}
该因子\textbf{只依赖键长与键角，完全不含二面角}——因为方位角 $\tau_i$ 只以
$\mathrm d\tau_i$ 的形式进入球坐标体积元，从不作为乘性权重出现。于是内坐标目标为
\begin{equation}
\mu_q(\mathrm dq)\propto e^{-\beta U_{\rm eff}(q)}\mathrm dq,\qquad
U_{\rm eff}(q)=U(x(q))-\frac1\beta J(q),\qquad
J(q)=\sum_i\bigl[2\ln b_i+\ln\sin a_i\bigr].
\end{equation}
在本实现中，锚定于 (ACE:C, ALA:N, ALA:CA) 的坐标系使精确的约化 Jacobian 为
$J(q)=\ln r_{23}+\sum_{\text{非根}}[2\ln b+\ln\sin a]$，其中 $\ln r_{23}$ 是第三个根原子
贡献的\emph{坐标架边界项}（二维极坐标元：键长的一次幂、无正弦因子）。
该式与 $60\times60$ 内 Jacobian 块的数值 $\log|\det|$ 吻合到 $\sim10^{-14}$，
且对任一二面角求导恰好为 $0$（机器零，非近似）。

\subsection{纯二面角弦的 Jacobian 抵消}

设 $r$ 是\textbf{纯二面角}位移（只在二面角槽位非零，例如只动 $\varphi$ 与 $\psi$）。
沿弦 $q-\theta r$，所有键长 $b_i$ 与键角 $a_i$ 都不变，故 $J(q-\theta r)=J(q)$ 对 $\theta$ 恒定，
于是
\begin{equation}
U_{\rm eff}(q-\theta r)-U_{\rm eff}(q)
=\bigl[U(x(q-\theta r))-U(x(q))\bigr]-\frac1\beta\bigl[J(q-\theta r)-J(q)\bigr]
=U(x(q-\theta r))-U(x(q))+0 .
\end{equation}
即打分的弦被积函数退化为\textbf{物理能量差}，修正项是 \emph{Jacobian-free} 的：
$e^{-\beta[U_{\rm eff}(q-\theta r)-U_{\rm eff}(q)]}=e^{-\beta[U(x(q-\theta r))-U(x(q))]}$。
（Jacobian 的梯度仍出现在扩散漂移 $\nabla_qU_{\rm eff}=\nabla_qU-\frac1\beta\nabla_qJ$ 中，
这是一个常规的有界项。）这一断言在 P3 被当作\emph{待检验命题}而非假设：实测残差
$4.6\times10^{-13}$（纯 $\varphi/\psi$）与 $2.8\times10^{-14}$（全部二面角），
残差量级只反映常数 $J$ 偏移在 float64 下的相消舍入。

\subsection{相关二面角（leader/offset）：一个必须修正的坐标缺陷}

若让共享同一转轴 $(C,B)$ 的多个原子各自持有\emph{绝对}二面角，则单独移动其中一个、
而固定其兄弟原子，改变的不是片段转动而是局部几何的\textbf{畸变}。实测：此约定下
$\varphi$ 方向的曲率高达 $822$ kJ/mol/rad$^2$（$\psi$ 为 $341$），因为改变 $\tau_{14}$
只转动 ALA:C 及其子树，却把 HA 与 CB 留在原地，破坏 C$\alpha$ 的四面体中心。
沿这样的方向做跳跃，正是上文的弦失效机制。

因此每条转轴上\textbf{最先放置}的原子作为 leader 持有真正的二面角，其兄弟原子持有
相对 leader 的（近似常数、很刚硬的）\textbf{偏移量}。移动 $\varphi$ 于是整体转动片段，
即物理的 Ramachandran 运动。该重参数化在二面角块上是\textbf{单位三角}的，
故 $|\det|$ 不变，上一节的 Jacobian 公式与二面角无关性\emph{原样成立}。
修正后 $\varphi$ 曲率降到 $79.9$、$\psi$ 降到 $33.0$。

\subsection{内坐标采样的定位与白化}

各向同性过阻尼 Langevin $\mathrm dq=-\nabla_qU_{\rm eff}\mathrm dt+\sqrt{2\varepsilon}\,\mathrm dW$
的不变测度是 $\mu_q\propto e^{-\beta U_{\rm eff}}$；再由 $q\mapsto x$ 推前，即得直角
Gibbs 测度 $\mu\propto e^{-\beta U}$——与 OpenMM 参考所采样的\textbf{同一个测度}。
因此 BAT 采样器给出的 $F(\varphi,\psi)$ 与直角参考给出的是同一个对象，比较是同量纲的。
需要明确的是：这是一个\emph{采样}构造，而不是内坐标下物理正确的动理学
（后者会带有位置相关的质量度规 $G(q)=J_x^\top J_x$ 与 Fixman 项）。
由于本文的研究对象是\emph{不变分布}，恒等度规的 Langevin 是正确且最简的选择，
并且对所有方法一视同仁地施加。

\textbf{刚性与预条件。} 键长/键角模式极硬，若在原始 $q$ 上用单一 $\Delta t$，步长会被
最快模式挟持。修正办法不去改动 \texttt{samplers.py}，而是像 E3/E4 把嵌入吸收进 $B$ 那样，
把一个\emph{常对角}白化吸收进坐标：令采样坐标为 $\tilde q=D^{-1}q$，
$D=\mathrm{diag}(\text{std}_i)$ 固定。$\tilde q$ 上的各向同性 Langevin 等价于 $q$ 上的
预条件 Langevin，保持 $\mu_{\tilde q}\propto e^{-\beta U_{\rm eff}(D\tilde q)}$，
并把各模式的时标拉齐。$D$ 取自\textbf{盆地极小点}处 $U_{\rm eff}$ Hessian 的对角
（所给构象并非驻点，$|\nabla U_{\rm eff}|=105$ 且几乎全部集中在 $\varphi$ 上，
直接取会把曲率虚高约 7 倍；先极小化落到 C5/$\beta$ 的 $(-147.5^\circ,159.9^\circ)$）。
只有 $\varphi,\psi$ 保持 $D=1$（它们必须仍是仿射、单位尺度的 CV，跳跃原子才能直接读作转动），
其余槽位一律缩放到公共热曲率 $\varepsilon=1/\beta$。白化后谱为 $0.56$--$87$，
最大曲率 $79.9$，故 $\Delta t=10^{-3}$ 时 $\Delta t\cdot H=0.08$；
用能量均分（$\langle U_{\rm eff}\rangle\approx U_{\min}+\tfrac d2 k_BT=167.4$、
std $=13.7$）核对，实测 $164.4$ 与 $13.5$。

\subsection{跳跃律、周期性与证书}

跳跃原子是纯二面角位移（只在 $\varphi,\psi$ 槽位非零），候选集取自由度 FES 各盆地极小点
之间的\emph{环面最短} $(\Delta\varphi,\Delta\psi)$，因而两个同伦方向（$\pm$ 对）天然成对出现；
随后逐一\textbf{筛除}：若某原子的弦穿过 Ramachandran 禁区，其打分指数会越过 $M_{\max}$，
该原子被丢弃并记录其几何与原因（对应 E3 删除直接 A--C 原子、E4 删除对角原子）；
未被直接连边的盆地对通过中间盆地中继。$h=0.1\min_a\|r_a\|$，CP 对的漂移步帽 $=2h$。

\textbf{周期性（实测结论与 §1.9 的假设不符，如实记录）。} $\varphi$ 仍取
$(-\pi,\pi]$：其接缝 $0.15$ rad 邻域内只有 $0.5\%$ 的参考质量。但 $\psi$ \emph{确实}
跨越接缝——丙氨酸的 C5/$\beta$ 区绕 $\psi=\pm180^\circ$ 连通，接缝邻域内有 $7.4\%$
的质量（$0.3$ rad 内 $17\%$），若不处理，欧氏 $W_2$/MMD/FES 会把 $\psi=+179^\circ$
与 $-179^\circ$ 当成最远。按 §2 的要求先\emph{报告}而非静默修补，随后把 $\psi$ 的
支割线移到 $-20^\circ$（$\alpha_R$ 与 C7eq 之间的空隙，实测密度 $1.7\times10^{-4}$），
即在窗口 $(-20^\circ,340^\circ]$ 上报告 $\psi$：接缝质量 $0.078\to0.005$。
这只是 E5 自己 \texttt{to\_cv} 里\emph{基本域}的选择，\texttt{metrics.py} 完全未动，
欧氏距离依然有效，且 C5/$\beta$ 盆地由此保持连通。盆地划分本身另外使用\textbf{环面}
Voronoi 度规，\texttt{TorusBox} 亦在物理单位下取模，二者都与窗口无关。

同样需要更正的是慢事件的路径：§1.9 假设低势垒路径过 $\varphi\approx0$，但对该力场
并非如此——$\varphi=0$ 分界线处为 $32.8$ kJ/mol，而 C7ax 岛自身的逃逸势垒仅为
$11.7$ kJ/mol（$4.67\,k_BT$），其路径绕 $\varphi\approx\pm180$。跳跃原子取环面最短位移，
因而自动走这条路径；$\tau_{\rm Kramers}$ 与 $\beta_{\min}$ 由\emph{盆地逃逸势垒}给出，
而不是某条 $\varphi$ 割线上的极小值。

\textbf{盆地必须是亚稳态。} FES 的原始极小点有 5 个，但其中两个的逃逸势垒为\emph{负}
（Voronoi 边界低于其自身极小），说明它们只是 $\beta$/C5 区的肩部而非亚稳态；
凡逃逸势垒低于 $1\,k_BT$ 的候选一律并入邻居。最终 $K=3$：
C5/$\beta$ $0.5766$、C7eq $0.4152$、C7ax $0.0082$。这一步不仅避免了 $p^\star$ 的碎片化，
更使“岛占据率”成为有意义的诊断——局域动力学本来就能自由到达肩部，
若把它算作岛，ULA 会被误判为已经翻越势垒。

由于状态空间是 60 维，无法使用张量求积网格，证书采用与 E4（24 维）相同的\textbf{移位形式}：
在漂移项内作变量替换 $x\to x+\theta_p r$ 后，残差退化为逐点 $\theta$ 求积缺陷的
自归一化平均，不再有 $O(1)$ 的相消留给 Monte Carlo；$\mu$ 期望直接对
重加权后的 metadynamics 样本池取。证书是\textbf{逐原子}的（比混合证书更强）。
检验函数取环面上\emph{周期}函数 $f=\sin(m\varphi+n\psi+c)$（tanh 脊在环面上不可容许），
并且与跳跃方向\textbf{对齐}——高维下随机脊满足 $a\cdot\hat r\sim1/\sqrt d$，对跳跃是盲的。
\textbf{直接形式}只报告不设门：其被积函数 $p\cdot S$ 恰好在 $p$ 指数小的地方是 $O(1)$，
在高维下不可由 $\mu$ 样本估计——这正是 E4 与 E5 都用移位形式作为定案证书的原因。

\subsection{参考分布}

参考来自对 $(\varphi,\psi)$ 的 well-tempered metadynamics（与 torch 模型\emph{同一个}柔性体系）。
偏置在平衡段增长，随后\textbf{冻结}，产出段按 $w(s)=\exp(\beta V_{\rm bias}(s))
=\exp(-\beta\frac{\gamma-1}{\gamma}F(s))$ 重加权回无偏 Boltzmann。
OpenMM 恰好返回 $F=-\frac{\gamma}{\gamma-1}V_{\rm bias}$，故该权重\emph{就是}所施加的偏置：
重加权对\textbf{任意静态偏置都是无偏的}，metadynamics 的收敛程度只影响方差、不影响偏倚。
参考按 R($\varphi$) 同样的纪律，被明确记录为\textbf{受收敛限制的实验输入}。

实现上有两个会静默出错的陷阱，均已设防：（i）OpenMM 以 \texttt{reversed(variables)}
分配偏置网格，\texttt{getFreeEnergy()} 的索引是 $[\psi,\varphi]$，在两轴网格数相等时转置\emph{不可见}，
不修正会得到关于原点镜像的 FES（全局极小落在 $(+157^\circ,-142^\circ)$ 而非 C5/$\beta$）；
（ii）$1\,$ns$=10^6\,$fs 的单位换算，一处 $1000/\Delta t_{\rm fs}$ 会让实际运行缩短 1000 倍
并把取帧步长舍入为零。前者额外用“帧直方图与 $e^{-\beta F/\gamma}$ 的相关性”作回归检验。
