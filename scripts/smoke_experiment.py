...
EXPERIMENT_NOTEBOOK_METHODS = {
    key: spec.methods_csv for key, spec in MANUSCRIPT_EXPERIMENTS.items()
}
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    # bounded real-dynamics smoke. Requiring set equality would prevent
    # expensive methods from running as separate method shards. A shard smokes
    # exactly the methods it runs,
...
def run_smoke(args: argparse.Namespace) -> dict:
    ...
    _write_json_exclusive(output_dir / "resolved_config.json", {
        **source_config,
        ...
        # Infinite box limits are legitimate scientific values but are not
        # strict JSON. Serialise them through the repository's sentinel
        # convention rather than relaxing allow_nan.
        "sampling_box": {
